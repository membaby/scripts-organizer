...
def startScraper(id):
   ...
   if len(run_time.split(':')[0]) < 2:
      run_time = '0' + run_time
   def run():
      if id not in running_processes:
         running_processes[id] = []

      print(1, running_processes)
      def start_scraper():
         print(2, running_processes)
         while running_processes[id]:
            print('TERMINATED')
            running_processes[id][0].terminate()
            running_processes[id].pop(0)
         ...
         print(cmd)
         updateSettings()
         running_processes[id].append(subprocess.Popen(cmd, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP))

      start_scraper()

      def restart_scraper():
         if cache['scrapers'][int(id)]['status']:
            for process in running_processes[id]:
               process.terminate()
            running_processes[id] = []
            start_scraper()
      ...
      if sche == 'Hourly':
         ...
         # schedule.every().minute.do(restart_scraper)
      
      
      while cache['scrapers'][int(id)]['status']:
         schedule.run_pending()
         if not running_processes[id][-1].poll() is None and cache['scrapers'][int(id)]['status'] != 2:
      ...
      try:
         running_processes[id][-1].terminate()
      except:
         pass
      while running_processes[id]:
         running_processes[id][0].terminate()
